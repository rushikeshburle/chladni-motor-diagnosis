...
function Diagnosis() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      setProcessingSteps(prev => [...prev, 'Validating inputs...'])
      ...
      const requestData = {
        ...
        temperature: motorInfo.temperature ? parseFloat(motorInfo.temperature) : null,
        image_path: imageData?.file_path || null,
        video_path: videoData?.file_path || null,
        text_input: textData || null,
        demo_mode: true
      }

      setProcessingSteps(prev => [...prev, 'Sending to analysis engine...'])
      ...
      if (result.success) {
        setProcessingSteps(prev => [...prev, 'Analysis complete!', 'Generating results...'])
        sessionStorage.setItem('diagnosisResults', JSON.stringify(result))
  ...
  const handleReset = () => {
    ...
    setVideoData(null)
    setTextData('')
    setMotorInfo({ motorId: '', motorType: '', rpm: '', load: '', temperature: '' })
    setError(null)
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-slide-up">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-4xl font-bold font-heading gradient-text mb-2">Motor Vibration Diagnosis</h1>
          <p className="text-gray-500">Upload vibration patterns and motor information for AI-powered analysis</p>
        </div>
        <button
          onClick={handleReset}
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          {[
            { key: 'motorId', label: 'Motor ID', placeholder: 'M-001' },
            { key: 'motorType', label: 'Motor Type', placeholder: 'Induction Motor' },
            { key: 'rpm', label: 'RPM', placeholder: '1450', type: 'number' },
            { key: 'load', label: 'Load (%)', placeholder: '75', type: 'number' },
            { key: 'temperature', label: 'Temperature (°C)', placeholder: '65', type: 'number' }
          ].map((field) => (
            <div key={field.key}>
              ...
              <input
                ...
                onChange={(e) => setMotorInfo({...motorInfo, [field.key]: e.target.value})}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <ImageUploader onData={setImageData} data={imageData} />
        <VideoUploader onData={setVideoData} data={videoData} />
        <TextInput onData={setTextData} data={textData} />
      </div>
      ...
      <div className="glass-card p-6 border border-[#F59E0B]/30">
        <div className="flex items-start space-x-3">
          ...
          <div>
            <p className="font-bold text-[#F59E0B] mb-1">Demo Mode Active</p>
            <p className="text-sm text-gray-500">
              Predictions are generated using demonstration logic. Connect a trained model for validated diagnosis.
            </p>
          </div>
        </div>
      </div>
